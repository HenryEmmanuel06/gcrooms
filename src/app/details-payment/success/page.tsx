...
export default async function DetailsPaymentSuccessPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  const params = await searchParams;
  const reference = params?.reference;
  const paymentId = params?.payment_id;
  
  if (!reference || !paymentId) {
    redirect('/details-payment/failed?error=missing_reference');
  }

  const base = await getBaseUrlFromHeaders();
  const res = await fetch(`${base}/api/details-payment/verify?payment_id=${encodeURIComponent(paymentId)}&reference=${encodeURIComponent(reference)}`, { cache: 'no-store' });

  if (!res.ok) {
    redirect('/details-payment/failed?error=not_found');
  }
  const data = await res.json();

  // Only allow rendering if verified status is success
  if (data.status !== 'success') {
    redirect(`/details-payment/failed?reference=${encodeURIComponent(reference)}&status=${encodeURIComponent(data.status || 'failed')}`);
  }
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              You will receive an email with the contact details shortly. Check your inbox!
            </p>
